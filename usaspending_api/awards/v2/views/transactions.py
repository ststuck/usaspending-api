...
class TestRoute53CnamesPocViewSet(APIView):
    @transaction.atomic(using="db_route53_cname")
    def get(self, request, format=None):
        """
        Test if we can round-robin to different DBs hitting the same DB url
        """
        logger.info("Starting request to TestRoute53CnamesPocViewSet")

        from django.db import connections
        connection = connections["db_route53_cname"]
        logger.info("Querying database via CNAME URL")
...
class TempEsTransactionHitViewSet(APIView):
    ...
    def get(self, request, format=None):
        ...
        build_temp_es_transaction_hits_by_city(scope="recipient_location", city="WASHINGTON", state_code="DC",
                                               country_code="USA", es_batch_size=50000, insert_chunk_size=25000)
        TempEsTransactionHitManager.index_temp_table()
