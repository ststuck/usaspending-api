        ...
        while not all_done:
            ...
            if self.average_chunk_seconds:
                ...
                if next_run_estimated_end_datetime >= options["closing_time"]:
                    ...
                    expected_completion = options["closing_time"] - next_run_estimated_end_datetime
                    logger.info("=> Expected completion time for next interation: {} ".format(expected_completion))
                    logger.info("=> Continuing with an estimated loop end datetime of: {}".format(dt_str))

            with Timer() as batch_timer:
                processed_count = self.time_runner(lower_datetime=week_end, upper_datetime=week_start)
            ...
            new_fy, new_fq = fiscal_year_and_quarter_from_datetime(week_end)
            if new_fy != curr_fy or new_fq != curr_fq:
                msg = "=== Completed FY{}Q{} with {} transactions ===\n"
                logger.info(msg.format(curr_fy, curr_fq, processed_count))
                logger.info("===== Running FY{}Q{} =====".format(new_fy, new_fq))
                curr_fy, curr_fq = new_fy, new_fq
    ...
    def time_runner(self, lower_datetime, upper_datetime):
        ...
        for batch_num, id_batch in enumerate(
            ...
            try:
                ...
                total_count_processed += len(id_batch)
            except SystemExit:
